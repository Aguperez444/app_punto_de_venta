from app.infrastructure.ui.tkinter.views.windows.find_bdd_window import FindBddWindow
from app.infrastructure.config.config_files_persistance import ConfigFilesPersistence

class ProgramStartController:
    def __init__(self):

        self.config_persistence_pointer = ConfigFilesPersistence()
    ...
    def verify_first_run(self):
        if self.config_persistence_pointer.is_first_time_running():
            self.find_bdd()


    def new_bdd_route_added(self, ruta):
        self.config_persistence_pointer.set_new_bbd_route_in_config(ruta)
