...
from app.infrastructure.ui.tkinter.controllers.actions.show_sales_controller import ShowSalesController
...
class ShowSalesListView(BaseProjectView):
    # ---------------------------------- ventana Principal de la clase ----------------------------------------
    def __init__(self, parent):
        super().__init__(parent)
        self.controller = ShowSalesController(self)
        # ----------------------------------------- ventana ---------------------------------------------
        ...
        self.calendario = ttk.DateEntry(self) # TODO cambiar por ttk.DateEntry en cuando corrijan el bug en prod.
    ...
    def realizar_busqueda_sale(self, _varname=None, _index=None, _mode=None):
        ...
        if fecha_del_dia == '':
            return
            # esto evita que se lance una excepción al cambiar entre fechas, ya que el cambiar de fecha
            # provoca que la fecha este vacía por un momento
        try:
            fecha_obj = datetime.strptime(fecha_del_dia, "%d/%m/%y")  # convertir string a datetime object
        except ValueError:
            fecha_obj = datetime.strptime(fecha_del_dia, "%d/%m/%Y")
    ...
    def on_resize(self, event):
        ...
        :return:
        """
        # Por algún motivo, si no se hace esto, al abrirse la ventana no ajusta bien el tamaño del cuadro. .-.
        if self.es_primer_resize:
            self.es_primer_resize = False
    ...
    def pasar_al_cuadro_ventas(self, ventas_list: list['Venta']):
        ...
        for venta in ventas_list:
            ...
            self.cuadro.insert("", "end", text=f'{venta.detalles[0].producto.nombre}...',
                               values=(f'{venta.detalles[0].cantidad}...' , f'{venta.total_price}', fecha, hora), tags=tag) # TODO, usar DTO's
            contador += 1
